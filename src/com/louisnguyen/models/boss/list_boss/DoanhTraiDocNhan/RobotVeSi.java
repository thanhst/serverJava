...
public class RobotVeSi extends TrungUyTrang {
   ...
   @Override
    public void joinMap() {
        if (zoneFinal != null) {
            joinMapByZone(zoneFinal, Util.nextInt(1300, 1567));
//            ChangeMapService.gI().changeMap(this, zoneFinal, , 312);
    ...
    public void reward(Player plKill) {
        if (Util.isTrue(100, 100)) {
            ItemMap it = new ItemMap(this.zone, 457, 2, this.location.x, this.zone.map.yPhysicInTop(this.location.x,
                    this.location.y - 24), plKill.id);
            for (Byte i = 0; i < 10; i++) {
                it = new ItemMap(this.zone, 457, 2, this.location.x, this.zone.map.yPhysicInTop(this.location.x,
                        this.location.y - 24), plKill.id);
            }
