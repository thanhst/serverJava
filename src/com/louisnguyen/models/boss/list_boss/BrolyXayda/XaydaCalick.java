...
public class XaydaCalick extends Boss {

    public XaydaCalick() throws Exception {
        super(2051, BossesData.BrolyXD, BossesData.SuperBrolyXayda);
    }

    @Override
    public void reward(Player plKill) {
        if (this.name.equals("Broly Xayda")) {
            if (Util.isTrue(100, 100)) {
                ItemMap caitrangBrolyNamec= new ItemMap(this.zone, 2050, 1, this.location.x,
                ...
                caitrangBrolyNamec.options.add(new Item.ItemOption(232, 2));
            ...
            if (Util.isTrue(5, 100)) {
                ItemMap caitrangBrolyNamec2 = new ItemMap(this.zone, 2050, 1, this.location.x,
                        this.zone.map.yPhysicInTop(this.location.x,
                                this.location.y - 24),
                        plKill.id);
                ...
                caitrangBrolyNamec2.options.add(new Item.ItemOption(232, 2));
                caitrangBrolyNamec2.options.add(new Item.ItemOption(160, 50));
                Service.getInstance().dropItemMap(this.zone, caitrangBrolyNamec2);
            }
        } else {
            if (Util.isTrue(100, 100)) {
                ItemMap caitrangBrolyNamec = new ItemMap(this.zone, 2051, 1, this.location.x,
                        this.zone.map.yPhysicInTop(this.location.x,
                                this.location.y - 24),
                        plKill.id);
                caitrangBrolyNamec.options.add(new Item.ItemOption(50, 23));
            ...
            if (Util.isTrue(5, 100)) {
                ItemMap caitrangBrolyNamec2 = new ItemMap(this.zone, 2051, 1, this.location.x,
                        this.zone.map.yPhysicInTop(this.location.x,
                                this.location.y - 24),
                        plKill.id);
                caitrangBrolyNamec2.options.add(new Item.ItemOption(50, 23));
