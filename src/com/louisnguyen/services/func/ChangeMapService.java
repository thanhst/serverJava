...
public class ChangeMapService {
    ...
    public void changeZone(Player pl, int zoneId) {
        ...
        if (!pl.isAdmin() || !pl.isBoss) {
            ...
            if ((Gio >= 18 && Gio <= 20) && (pl.zone.map.mapId >= 68 && pl.zone.map.mapId <= 72)
                    && (pl.playerTask.taskMain.id != 21)) {
                Service.gI().sendThongBaoOK(pl, "Không thể đổi khu vực vì đang trong thời gian hỗ trợ nhiệm vụ");
                return;
            }
            if ((Gio >= 18 && Gio <= 20) && (pl.zone.map.mapId >= 63 && pl.zone.map.mapId <= 67)
                    && (pl.playerTask.taskMain.id != 21)) {
                Service.gI().sendThongBaoOK(pl, "Không thể đổi khu vực vì đang trong thời gian hỗ trợ nhiệm vụ");
                return;
            }
            if ((Gio >= 18 && Gio <= 20) && (pl.zone.map.mapId >= 73 && pl.zone.map.mapId <= 77)
                    && (pl.playerTask.taskMain.id != 21)) {
                Service.gI().sendThongBaoOK(pl, "Không thể đổi khu vực vì đang trong thời gian hỗ trợ nhiệm vụ");
                return;
            }
            if ((Gio >= 18 && Gio <= 20) && (pl.zone.map.mapId >= 81 && pl.zone.map.mapId <= 83)
                    && (pl.playerTask.taskMain.id != 22)) {
                Service.gI().sendThongBaoOK(pl, "Không thể đổi khu vực vì đang trong thời gian hỗ trợ nhiệm vụ");
                return;
            }
    ...
    public void changeMapWaypoint(Player player) {
        // System.out.println("Change way point");
        Zone zoneJoin = null;
        WayPoint wp = null;
    ...
    public void finishLoadMap(Player player) {
        ...
        // Logger.log(Logger.BLUE, "Bạn " + player.name + " đang ở " +
    ...
    public void goToMap(Player player, Zone zoneJoin) {
        ...
        // System.out.println("ChangeMapservice"); toàn cái này chạy!!
    }
    ...
    public Zone checkMapCanJoin(Player player, Zone zoneJoin) {
        ...
        if (zoneJoin != null) {
            switch (zoneJoin.map.mapId) {
                ...
                // Service.gI().sendThongBao(player, "Bạn chưa đủ sức mạnh để tới được khu vực
                // này!");
                // return null;
                // }
                // break;
